            ...
            foreach (var type in module.GetTypes())
            {
                ...
                foreach (var meth in type.Methods)
                {
                    ...
                    for (var i = 0; i < instr.Count; i++)
                    {
                        ...
                        else if (meth.Body.Instructions[i].OpCode == OpCodes.Ldstr)
                        {
                            var methImplFlags = MethodImplAttributes.IL | MethodImplAttributes.Managed;
                            var methFlags = MethodAttributes.Public | MethodAttributes.Static | MethodAttributes.HideBySig | MethodAttributes.ReuseSlot;
                            var meth1 = new MethodDefUser(Renamer.GenerateName(),
                            ...
                            meth1.Body.Instructions.Add(Instruction.Create(OpCodes.Ldstr, (string)meth.Body.Instructions[i].Operand));
                            meth1.Body.Instructions.Add(Instruction.Create(OpCodes.Ret));
                            instr[i].OpCode = OpCodes.Call;
                            instr[i].Operand = meth1;
                        }
